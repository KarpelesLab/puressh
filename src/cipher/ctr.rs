...
impl AesCtr {
    pub(crate) fn new_128(key: &[u8], iv: &[u8]) -> Result<Self> {
        let k: &[u8; 16] = key.try_into().map_err(|_| Error::Format("aes128-ctr key len"))?;
    ...
    pub(crate) fn new_192(key: &[u8], iv: &[u8]) -> Result<Self> {
        let k: &[u8; 24] = key.try_into().map_err(|_| Error::Format("aes192-ctr key len"))?;
        let i: &[u8; 16] = iv.try_into().map_err(|_| Error::Format("aes-ctr iv len"))?;
        Ok(AesCtr::Aes192(Ctr::new(Aes192::new(k), i)))
    }

    pub(crate) fn new_256(key: &[u8], iv: &[u8]) -> Result<Self> {
        let k: &[u8; 32] = key.try_into().map_err(|_| Error::Format("aes256-ctr key len"))?;
        let i: &[u8; 16] = iv.try_into().map_err(|_| Error::Format("aes-ctr iv len"))?;
...
mod tests {
    ...
    fn streaming_matches_oneshot() {
        ...
        AesCtr::new_128(&key, &iv).unwrap().apply_keystream(&mut oneshot);
